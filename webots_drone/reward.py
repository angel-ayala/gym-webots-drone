...
def compute_target_orientation_reward(ref_position, position, orientation,
                                      threshold_offset=0.015, n_segments=3):
    ...
    for n in range(n_segments):
        if curr_distance >= 1 - threshold_offset * n * 1.5:
            r_distance *= 2.

    # normalize
    r_distance /= 2**n_segments
    if curr_distance < 0.95:
        r_distance -= 0.1
...
def compute_distance_reward(distance, d_central=None, distance_threshold=25.,
    ...
    if curr_distance < distance_threshold - threshold_offset:
        r_distance -= 1.
    # bonus in-distance
    elif curr_distance < distance_threshold:
        r_distance += 1.
...
def normalize_and_mul(distance_rewards, orientation_rewards):
    ...
    r_sum = r_orientation * r_distance
    return r_sum
...
def compute_position2target_reward(ref_position, pos_t, pos_t1, orientation_t1,
    ...
    dist_t = compute_distance(ref_position, pos_t) - d_central
    dist_t1 -= d_central
    dist_factor = compute_distance_diff_scalar(dist_t1 - dist_t)
    r_sum = np.abs(r_sum) * dist_factor
    return r_sum
...
if __name__ == '__main__':
    ...
    def plot_derivatives(derivatives,
                         ...
                         plot_all=False, norm=True, title=""):
        ...
        for d in directions:
            ...
            if plot_all:
                plot_reward_heatmap(x_grid, y_grid, derivatives[d_idx],
                                    f"Derivative {d} direction{title}")
        # sum
        ...
        if norm:
            ...
            plot_reward_heatmap(x_grid, y_grid, dist_derivatives_norm,
                                f"Derivative norm. {s_dir} direction{title}")
        else:
            dist_derivatives_norm = dist_derivatives_total
            plot_reward_heatmap(x_grid, y_grid, dist_derivatives_total,
                                f"Derivative {s_dir} direction{title}")

        return dist_derivatives_norm
    ...
    plot_reward_heatmap(x_grid, y_grid, -p_distance/36.5, 'Position distances, target centered')
    ...
    # Plot derivatives
    d_dist_ori = calculate_distance_derivative(x_grid, y_grid, r_dist_ori)
    d_dist_ori_norm = plot_derivatives(d_dist_ori, ['up', 'left'],
                                       title=" direct distance and orientation reward")
    # d_dist_ori_norm = plot_derivatives(d_dist_ori, ['right', 'down'])
    # distance
    d_distance = calculate_distance_derivative(x_grid, y_grid, r_distance)
    d_distance_norm = plot_derivatives(d_distance, ['up', 'left'],
                                       title=" distance reward")
    # d_distance_norm = plot_derivatives(d_distance, ['right', 'down'])
    # orientation
    d_orientation = calculate_distance_derivative(x_grid, y_grid, r_orientation)
    d_orientation_norm = plot_derivatives(d_orientation, ['up', 'left'],
                                       title=" orientation reward")
    # d_orientation_norm = plot_derivatives(d_orientation, ['right', 'down'])
    # distance + orientation
    d_dist_ori = normalize_and_mul(d_distance_norm, d_orientation_norm)
    plot_reward_heatmap(x_grid, y_grid, d_dist_ori,
                        'Derivative distance and orientation rewards2')
    # target approximation
    plot_reward_heatmap(x_grid, y_grid, p_distance, 'Position distances')
    d_pos = calculate_distance_derivative(x_grid, y_grid, p_distance - d_central)
    d_pos_up = plot_derivatives(d_pos, ['up', 'left'], title=" position positive")
    d_pos_down = plot_derivatives(d_pos, ['down', 'right'], title=" position negative")
    up_factor = compute_distance_diff_scalar(d_pos_up)
    down_factor = compute_distance_diff_scalar(d_pos_down)
    plot_reward_heatmap(x_grid, y_grid, np.abs(r_dist_ori) * up_factor,
                        'Derivative dist&ori times up_factor')
    plot_reward_heatmap(x_grid, y_grid, np.abs(r_dist_ori) * down_factor,
                        'Derivative dist&ori times down_factor')
    compose = np.abs(r_dist_ori) * down_factor + np.abs(r_dist_ori) * up_factor
    plot_reward_heatmap(x_grid, y_grid, compose, 'Derivative dist&ori times up_factor&down_factor')
    d_compose = calculate_distance_derivative(x_grid, y_grid, compose)
    d_compose_norm = plot_derivatives(d_compose, ['up', 'left'])
    d_compose_norm = plot_derivatives(d_compose, ['down', 'right'])
